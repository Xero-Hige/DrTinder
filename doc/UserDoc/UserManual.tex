...
\node [mybox] (box){%
    \begin{minipage}{\textwidth}
        \emph{Todos sabemos que es difícil recordar lo que se converso (mas si se hace con muchas personas en simultaneo). Entonces se provee de un botón (como se ve en la Figura \ref{fig:chatDown}) que permite volver hasta el ultimo mensaje luego de scrolear por todo el historial.}
    \end{minipage}
};
\node[fancytitle, right=10pt, rounded corners] at (box.north west){$\spadesuit$ \textbf{Consejo}};
\end{tikzpicture}
